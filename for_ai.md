# AMS (asuna-memory) v2.7.2 — AI Agent Execution Script
...
**Verify:** `./asuna-memory --version` → a line `asuna-memory 2.x.y` (today `asuna-memory 2.7.2`). Record the exact number as `<X>` — §3.2/§5.1/§6 expect it. No such line (error page saved as file, corrupt archive) → STOP.
...
./target/release/asuna-memory --version   # must equal this checkout's Cargo.toml `version` (main today: 2.7.2); record it as <X>
```
