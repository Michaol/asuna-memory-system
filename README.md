...
v2.7.3 is a patch release: **no breaking changes, no config changes, no data migration** — replace the binary and restart. Contents: **release gate** (tag builds now require fmt + clippy + tests green before publishing assets); **unified `bounded_memory` writer** (`insert_memory_row` for atom / scenario / supersede / write / reconcile / split — confidence bucketing, `edited_at`, and `updated_at` live in one place); **docs honesty** (architecture / code-review / aegis specs marked ARCHIVED with v2.7 corrections); supply-chain pin for `requests` in `hermes-plugin/install.sh`; `recover_poison` moved to `util` (service no longer depends on `transport`); tokio features narrowed from `full`. Full changelog: [HISTORY.md](HISTORY.md).
