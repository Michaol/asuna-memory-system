...
|---|---|---|---|
| Rust ≥ 1.85 | `rustc --version` | `rustc 1.85+` | source build only (`rust-version = "1.85"`) |
...
|---|---|---|---|
| MSRV | cargo: `cannot be built because it requires rustc 1.85.0 or newer`（或依赖报 `feature edition2024 is required`） | old toolchain | `rustup update stable`, rebuild |
